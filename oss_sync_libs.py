...
class OssOperation(object):  # TODO 使用@retry重写重试部分
    ...
    def __init__(self, kms_SK=None):
        oss2.set_file_logger(config.LogFile, 'oss2', config.LogLevel)
        if not kms_SK:
            kms_SK = str(getpass("请输入AK为\"%s\"的KMS服务的SK：" % color.red(config.KMSAccessKeyId)))
        ...
        self.__bucket = oss2.CryptoBucket(
            ...
            crypto_provider=oss2.crypto.AliKMSProvider(config.KMSAccessKeyId, kms_SK, config.KMSRegion, config.CMKID)
        )
        try:  # 检测Bucket是否存在
        ...
        try:  # 检测KMS配置有效性
            KmsClient(open_api_models.Config(access_key_id=config.KMSAccessKeyId, access_key_secret=kms_SK, endpoint='kms.%s.aliyuncs.com' % config.KMSRegion)).generate_data_key(kms_models.GenerateDataKeyRequest(key_id=config.CMKID))
        except:
    ...
    def upload_file_encrypted(self, local_file_name, remote_object_name, storage_class='Standard', file_sha256=None, cache_control='no-store', check_sha256_before_upload=False):
        """使用KMS加密并上传文件
        ...
        Args:
            ...
            check_sha256_before_upload (bool, 可选): 是否在上传之前对比远端文件的sha256，如相同则跳过上传
        """
        if not file_sha256:
            file_sha256 = calculate_local_file_sha256(local_file_name)
        retry_count = 0
        if check_sha256_before_upload:
            try:
            ...
            if remote_object_sha256 == file_sha256:
                logger.info("[Upload_File_Encrypted]sha256相同，跳过%s文件的上传" % local_file_name)
                return 200
        while True:
            try:
    ...
    def Download_Decrypted_File(self, local_file_name, remote_object_name, version_id=None):
        """从OSS下载并解密文件
    ...
    def Copy_remote_files(self, copy_list: dict, storage_class='Standard'):
        ...
        Args:
            ...
            copy_list (dits): Key:目标文件, value:源文件
        """
    ...
    def Get_Remote_File_Meta(self, remote_object: str, version_id=None):
        """获取一个远程文件的元信息
        ...
        try:
            if version_id:
                objectmeta = self.__bucket.head_object(remote_object, params={'versionId': version_id})
            else:
